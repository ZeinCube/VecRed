...
public class RoundRect extends Figure {
    ...
    public boolean isContainPoint(Point p) {
        ...
        Rectangle shape  = new javafx.scene.shape.Rectangle(Math.min(canvSt.x,canvEnd.x)-(size*Controller.scaleSize/2)/2,Math.min(canvSt.y,canvEnd.y)-(size*Controller.scaleSize/2)/2,Math.abs((canvSt).x-(canvEnd).x)+(size*Controller.scaleSize/2),Math.abs((canvSt).y-(canvEnd).y)+(size*Controller.scaleSize/2));
    ...
    public void draw(GraphicsContext graphicsContext) {
        ...
        if(!isFilled){
            graphicsContext.strokeRoundRect(Math.min(canvSt.x,canvEnd.x),Math.min(canvSt.y,canvEnd.y),Math.abs(canvSt.x-canvEnd.x),Math.abs(canvSt.y-canvEnd.y),size,size);
        }else{
            graphicsContext.fillRoundRect(Math.min(canvSt.x,canvEnd.x),Math.min(canvSt.y,canvEnd.y),Math.abs(canvSt.x-canvEnd.x),Math.abs(canvSt.y-canvEnd.y),size,size);
            graphicsContext.strokeRoundRect(Math.min(canvSt.x,canvEnd.x),Math.min(canvSt.y,canvEnd.y),Math.abs(canvSt.x-canvEnd.x),Math.abs(canvSt.y-canvEnd.y),size,size);
        }
        if(isSelected){
            drawSelection();
